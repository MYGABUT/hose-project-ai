...
import { getDashboardSummary, getActiveJobs, getLowStockAlerts } from '../../services/analyticsApi';
...
export default function Dashboard() {
    ...
    // Local state for other specific dashboard needs if any, 
    // but mostly we should use context data now.
    ...
    const loadDashboardData = async () => {
        ...
        setError(null);
        try {
            // Only fetch non-analytics data here if needed (e.g., activeJobs, alerts)
            // Summary and Production Stats come from Context now
    ...
    return (
        <div className="dashboard">
            <div className="dashboard-header">
                ...
                <p className="page-subtitle">Ringkasan aktivitas produksi dan inventori</p>
            </div>
                ...
                {productionStats?.byMachine?.length > 0 ? (
                    productionStats.byMachine.map((machine, idx) => (
                        <MachineStatusWidget
                            ...
                            serviceInterval={machine.target || 15000} // Assuming target is interval for now
                    ...
                    // Fallback if no specific machine data, but maybe we want to show one default connected to summary
                    <MachineStatusWidget
